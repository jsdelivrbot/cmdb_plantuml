...
(function(Buffer, process) {
    ...
    var lines = str.split('\n').map(function(line) {
      ...
      for (i = 0; i < len; i += 1) {
        ...
        if (!~origin && ~lastOrigin) {
          ...
          if (lastOrigin + 1 in nameLocations)
            nameIndex = names.indexOf(nameLocations[lastOrigin + 1]);
  ...
  var MagicString = (function() {
    function MagicString(string) {
      ...
      Object.defineProperties(this, {
        ...
        mappings: {
          writable: true,
          value: initMappings(string.length)
        },
        filename: {
          writable: true,
  ...
  pp$5.initialContext = function() {
    return [types.b_stat];
  };
  pp$5.braceIsBlock = function(prevType) {
  ...
  pp$5.updateContext = function(prevType) {
  ...
  pp$6.getToken = function() {
    this.next();
    return new Token(this);
  };
  if (typeof Symbol !== "undefined")
    pp$6[Symbol.iterator] = function() {
      var self = this;
  ...
  pp$6.setStrict = function(strict) {
    this.strict = strict;
  ...
  pp$6.readToken = function(code) {
    if (isIdentifierStart(code, this.options.ecmaVersion >= 6) || code === 92)
      return this.readWord();
    return this.getTokenFromCode(code);
  };
  pp$6.fullCharCodeAtPos = function() {
  ...
  pp$6.skipBlockComment = function() {
  ...
  pp$6.skipLineComment = function(startSkip) {
    var start = this.pos;
    var startLoc = this.options.onComment && this.curPosition();
  ...
  pp$6.finishToken = function(type, val) {
    this.end = this.pos;
    if (this.options.locations)
  ...
  pp$6.readToken_mult_modulo = function(code) {
    var next = this.input.charCodeAt(this.pos + 1);
    if (next === 61)
      return this.finishOp(tt.assign, 2);
    return this.finishOp(code === 42 ? tt.star : tt.modulo, 1);
  };
  pp$6.readToken_pipe_amp = function(code) {
    var next = this.input.charCodeAt(this.pos + 1);
    if (next === code)
  ...
  pp$6.readToken_caret = function() {
    var next = this.input.charCodeAt(this.pos + 1);
    if (next === 61)
      return this.finishOp(tt.assign, 2);
    return this.finishOp(tt.bitwiseXOR, 1);
  };
  pp$6.readToken_plus_min = function(code) {
    var next = this.input.charCodeAt(this.pos + 1);
    if (next === code) {
  ...
  pp$6.readToken_lt_gt = function(code) {
    var next = this.input.charCodeAt(this.pos + 1);
    var size = 1;
    if (next === code) {
  ...
  pp$6.readToken_eq_excl = function(code) {
    var next = this.input.charCodeAt(this.pos + 1);
    if (next === 61)
  ...
  pp$6.getTokenFromCode = function(code) {
    switch (code) {
      case 46:
  ...
  pp$6.finishOp = function(type, size) {
  ...
  pp$6.readInt = function(radix, len) {
    var start = this.pos,
        total = 0;
    for (var i = 0,
  ...
  pp$6.readRadixNumber = function(radix) {
    this.pos += 2;
  ...
  pp$6.readNumber = function(startsWithDot) {
    var start = this.pos,
        isFloat = false,
  ...
  pp$6.readString = function(quote) {
    var out = "",
        chunkStart = ++this.pos;
    for (; ; ) {
      if (this.pos >= this.input.length)
  ...
  pp$6.readEscapedChar = function(inTemplate) {
    var ch = this.input.charCodeAt(++this.pos);
    ++this.pos;
    switch (ch) {
      case 110:
  ...
  var Declaration = (function() {
    ...
    Declaration.prototype.render = function render(es6) {
      if (es6)
        return this.name;
      if (!this.isReassigned || !this.isExported)
        return this.name;
  ...
  var SyntheticNamespaceDeclaration = (function() {
    ...
    SyntheticNamespaceDeclaration.prototype.addReference = function addReference(reference) {
      if (reference.parts.length) {
        ...
        reference.end += reference.name.length + 1;
      ...
      walk(this.node, {
        enter: function(node, parent) {
          ...
          if (node.type === 'Property' && node.shorthand && parent.type !== 'ObjectPattern') {
            ...
            reference.isShorthandProperty = true;
      ...
      this.statements.forEach(function(statement) {
        ...
        statement.references.forEach(function(reference) {
          ...
          if (declaration) {
            ...
            if (reference.name === _name2 && _name2.length === end - start)
              return ;
    ...
    Bundle.prototype.render = function render() {
      ...
      if (options.sourceMap) {
        ...
        if (this.transformers.length)
          map = collapseSourcemaps(map, usedModules);
    ...
    Bundle.prototype.sort = function sort() {
      ...
      function visit(module) {
        ...
        function addStrongDependencies(dependency) {
          if (stronglyDependsOn[module.id][dependency.id])
            return ;
  ...
  var VERSION = '0.21.3';
  var ALLOWED_KEYS = ['entry', 'dest', 'plugins', 'external', 'onwarn', 'indent', 'format', 'moduleName', 'sourceMap', 'intro', 'outro', 'banner', 'footer', 'globals', 'transform', 'load', 'resolveId', 'resolveExternal'];
