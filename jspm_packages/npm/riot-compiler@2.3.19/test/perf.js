...
(function(process) {
  'use strict';
  var compiler23 = require("../dist/compiler").compile,
      compiler22 = require("./v223/compiler223").compile,
      tmpl23 = require("riot-tmpl").tmpl,
      tmpl22 = require("./v223/compiler223").tmpl,
      assert = require("assert"),
      path = require("path"),
      fs = require("fs");
  var basedir = path.join(__dirname, 'specs', 'fixtures'),
  ...
  console.log('======================================================================');
  console.log();
  console.log('Testing each compiler with %d files, %dx%d iterations (%d total)', tags.length, TMAX - 2, LOOP, tags.length * (TMAX - 2) * LOOP);
  ...
  console.log();
  console.log('%s:%s%s', padr('Memory used', CPAD), padl(mem22.heapUsed.toLocaleString(), CPAD), padl(mem23.heapUsed.toLocaleString(), CPAD));
  console.log();
  console.log('NOTES:');
