...
describe('Compile tags', function() {
  ...
  it('Parsing the options attribute in script tags', function() {
    var src = cat(fixtures, 'script-options.tag'),
  ...
  it('The `whitespace` option preserves newlines and tabs', function() {
    var src = ['<whitespace>', '\t<p>xyz', '\t  cc', '\t    ss</p>', '</whitespace>'].join('\n'),
  ...
  it('The `entities` option give access to the compiled parts', function() {
    var parts = compiler.compile(cat(fixtures, 'treeview.tag'), {entities: true}),
        resarr = [['treeview', /^<ul id="treeview"> <li> <treeitem data="\{treedata}">/, '', '', /\s+this.treedata = {/], ['treeitem', /^<div class="\{bold: isFolder\(\)}" onclick="\{toggle}"/, '', '', /\s+var self = this\s+self.name = opts.data.name/]];
  ...
  it('The `exclude` option to ignore parts of the tag', function() {
    var parts = compiler.compile(cat(fixtures, 'treeview.tag'), {
      ...
      exclude: ['html', 'js']
        ...
        dummyTag = ['<my-tag>', '<p>{ hi }</p>', 'this.hi = "hi"', '<style scoped>', ' :scope { color: red; }', '</style>', '</my-tag>'].join('\n');
    expect(compiler.compile(dummyTag, {exclude: ['html']})).to.be("riot.tag2('my-tag', '', 'my-tag,[riot-tag=\"my-tag\"] { color: red; }', '', function(opts) {\nthis.hi = \"hi\"\n\n});");
    expect(compiler.compile(dummyTag, {exclude: ['html', 'js']})).to.be("riot.tag2('my-tag', '', 'my-tag,[riot-tag=\"my-tag\"] { color: red; }', '', function(opts) {\n});");
