...
(function(process) {
  ...
  var data = {
    ...
    item: null
      ...
      exprList = [['{ date }', data.date], ['{ num === 0 ? 0 : num }', data.num], ['<p>{str}</p>', '<p>string</p>'], [' "{ str.slice(0, 3).replace(/t/, \'T\') }" ', ' "sTr" '], ['{this.num}', 1], ['{ !bool }', false], ['{}', undefined]],
      csList = [['{ foo: num }', 'foo'], ['{ foo: num, bar: item }', 'foo'], ['{ foo: date.getFullYear() > 2000, bar: str==this.str }', 'foo bar']],
      ex22a,
      ex22b,
      mem22,
      tt22 = [],
